...
@section('admin_content')
    ...
    <div class="box span12">
        ...
        <div class="box-content">
            ...
            <form class="form-horizontal" method="post" action="{{url('/update_product/'.$product->id)}}" enctype="multipart/form-data">
                <fieldset>
                    {{csrf_field()}}
                        ...
                        <div class="controls">
                            <select name="category_id" class="form-control">
                                @foreach ($categories as $category)
                                    <option value="{{$category->id}}" @if($category->id==$product->category_id) selected @endif>{{$category->name}} ({{$category->parent->name}})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <textarea class="form-control" rows="5" name="long_description" required="">{{$product->long_description}}</textarea>
                        </div>
                    </div>
